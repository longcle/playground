...
class WP_JSON_RPC_Server extends IXR_Server
{
	...
	function regUser($args){
		...
		if (username_exists($phone_number)){
			$res['error_msg'] = 'Ðang ký không thành công: Tài kho?n dã t?n t?i.';
		} else {
	...
	function loginUser($args){
		...
		if (count($users) > 0){
			...
			$user = wp_signon($creds,false);
			if (is_wp_error($user)){
				$res['error_msg'] = 'Ðang nh?p không thành công. Xin th? l?i.';
			} else {
				global $wpdb;
			...
			$res['error_msg'] = 'S? di?n tho?i không h?p l?.';
		}
		...
		foreach ($posts as $post){
            ...
            //get events with exp_date <= 7
                ...
                foreach ($events as $event) {
                    ...
                    $exp_date = $event->pg_event_expire_date;
                    if (trim($exp_date) != "") {  
                        $arrExp = explode('-', $exp_date);
                        $check = checkdate($arrExp[1], $arrExp[2], $arrExp[0]);
                        if ($check) {
                            $curr_date = date('Y-m-d');
                            $days = floor( (abs(strtotime($exp_date) - strtotime($curr_date)))/(60*60*24) );
                            if ($days >= 0 && $days <= 7) {
	...
	function get_custom_fields($post_id) {
		...
		$metas = $wpdb->get_results( $wpdb->prepare("SELECT meta_key, meta_value, meta_id, post_id
				FROM $wpdb->postmeta WHERE post_id = %d
				ORDER BY meta_key,meta_id", $post_id), ARRAY_A );
	...
	function wp_getTerms( $args ) {
		...
		if ( isset( $filter['parent'] ) )
			$query['parent'] = absint($filter['parent']);
		if ( isset( $filter['child_of'] ) )
			$query['child_of'] = absint( $filter['child_of'] );
